...
struct vfsmount *devpts_mntget(struct file *filp)
{
	...
	/* Is the mount already a devpts mount? */
	if (filp->f_vfsmnt->mnt_sb->s_magic == DEVPTS_SUPER_MAGIC) {
	...
	filp2 = file_open_root(filp->f_dentry->d_parent, filp->f_vfsmnt,
			       "pts/ptmx", O_NOFOLLOW | O_PATH);
	if (IS_ERR(filp2))
		goto out;
	/* Is pts/ptmx on devpts? */
	if (filp2->f_vfsmnt->mnt_sb->s_magic == DEVPTS_SUPER_MAGIC)
		mnt = mntget(filp2->f_vfsmnt);
	fput(filp2);
