...
static int ocfs2_write_dquot(struct dquot *dquot)
{
	...
	trace_ocfs2_write_dquot(
		from_qown(&init_user_ns, dquot->dq_type, dquot->dq_id),
		dquot->dq_type);
...
static int ocfs2_acquire_dquot(struct dquot *dquot)
{
	...
	trace_ocfs2_acquire_dquot(
		from_qown(&init_user_ns, type, dquot->dq_id), type);
	mutex_lock(&dquot->dq_lock);
	/*
