...
int vfs_parse_fs_param(struct fs_context *fc, struct fs_parameter *param)
{
	...
	if (strcmp(param->key, "subtype") == 0) {
		if (!(fc->fs_type->fs_flags & FS_HAS_SUBTYPE))
			return invalf(fc, "VFS: subtype not supported");
		if (param->type != fs_value_is_string)
			return invalf(fc, "VFS: Non-string subtype");
		if (fc->subtype)
			return invalf(fc, "VFS: Multiple subtype");
				      ...
				      enum fs_context_purpose purpose)
{
	...
	/* Needed until all filesystems are converted */
...
int validate_fc(struct fs_context *fc)
{
	if (fc->ops->validate) {
		int ret = fc->ops->validate(fc);
		if (ret < 0)
			return ret;
	}
...
static int legacy_get_tree(struct fs_context *fc)
{
	...
	if (data && !(fc->fs_type->fs_flags & FS_BINARY_MOUNTDATA)) {
	...
	root = fc->fs_type->mount(fc->fs_type, fc->sb_flags, fc->source, data);
	error = PTR_ERR(root);
	if (IS_ERR(root))
		goto out_free_secdata;
	...
	error = security_sb_kern_mount(sb, fc->sb_flags, secdata);
	if (error)
		goto out_sb;
...
static int legacy_reconfigure(struct fs_context *fc)
{
	...
	return sb->s_op->remount_fs(sb, &fc->sb_flags, data);
}
